...
static void vHandlerTask(void *pvParameters)
{
    for (;;)
    {
        ...
        {
            vPrintString("Handler task - Processing event1.\r\n");
        }
    }
}
static void vHandlerTask1(void *pvParameters)
{
    for (;;)
    {
        ...
        if (xSemaphoreTake(xBinarySemaphore, portMAX_DELAY) == pdTRUE)
        {
            vPrintString("Handler task - Processing event2.\r\n");
        }
    }
}
int main(void)
{
    ...
    if (xTaskCreate(vHandlerTask1, "Handler Task", 1000, NULL, 3, &xHandlerTask) != pdPASS)
       {
           CY_ASSERT(0);  // Task creation failed, stop here
       }
    ...
    for (;;);
//    when the interrupt is called then it exexute isr it do only necessary performance and other execution can be done by handle function then interrupt return to task in which it calls  execute
}

...
static void vHandlerTask1(void *pvParameters)
{
    for (;;)
    {
        ...
        vTaskPrioritySet( xHandlerTask2,priority+1);
    }
...
static void vHandlerTask2(void *pvParameters)
{
    for (;;)
    {
        ...
        if (xSemaphoreTake(xCountingSemaphore, portMAX_DELAY) == pdTRUE)
        {
            vPrintString("Handler task 2 - Processing event.\r\n");
        }

        UBaseType_t priority = uxTaskPriorityGet(NULL);
              vTaskPrioritySet( xHandlerTask2,priority-2);
    }
...
int main(void)
{
    ...
    // Create handler tasks
       if (xTaskCreate(vHandlerTask2, "Handler Task 1", 1000, NULL, 3, &xHandlerTask2) != pdPASS)
       {
           CY_ASSERT(0);  // Task creation failed, stop here
       }
    // Create the periodic task

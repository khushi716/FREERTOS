...
#define MBUS_READ_ADC_VAL_ADDR 1
...
static regMap_t tHoldingRegMap[] = {
    ...
    { MBUS_READ_ADC_VAL_ADDR, APP_ONE_BYTE_VAL_LENGTH, &tMesurementsResult.adc_v, cbCmdControlLightIntensity,NULL},
...
{cy_rslt_t result;
    ...
    if ((APP_TWO_BYTE_VAL_LENGTH == bDataLength) && (pwm_control_value >= 0 && pwm_control_value <= 100))
    {
        ...
        tMesurementsResult.adc_v = (uint8_t)(pwm_control_value);
        ...
        result = cyhal_pwm_set_duty_cycle(&pwm_led_control, duty_cycle, PWM_FREQUENCY);
